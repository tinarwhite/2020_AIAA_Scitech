...
def build_network(n_io, savename, n_hidden, n_clusters, learn_type, nonlin, learn_vars, lr_vars, train_state, norm_type):
    ...
    loss_runs, reg_loss = define_loss(y_, y, c, f, learn_type, train_state, n_clusters, norm_type)
    ...
    return sess, input, y_, f, c, y, loss_runs, reg_loss, train_step_runs, learning_rate
...
def network_architecture(n_io, n_clusters,n_hidden,n_c_hidden,learn_type,nonlin):
    ...
    if learn_type == 'mlpsc': 
        print('Building mlpsc network architecture...')
        c, f = (tf.ones_like(y_), y_) # dummy values
        y = single_mlpsc_network(input, [n_io[0]] + n_hidden + [n_io[1]], nonlin[0])
    if learn_type == 'mlpec': 
        print('Building mlpec network architecture...')
        c, f = (tf.ones_like(y_), y_) # dummy values
        y = single_mlpec_network(input, [n_io[0]] + n_hidden + [n_io[1]], nonlin[0])
    if learn_type == 'resnet': 
        print('Building resnet network architecture...')
        c, f = (tf.ones_like(y_), y_) # dummy values
        y = single_mlp_network(input, [n_io[0]] + n_hidden + [n_io[1]], nonlin[0], resnet = 'on')
    if learn_type == 'mlsp': 
        print('Building mlsp network architecture...')
        c, f = (tf.ones_like(y_), y_) # dummy values
        y = single_mlsp_network(input, [n_io[0]] + n_hidden + [n_io[1]], nonlin[0])
...
def cluster_architecture(n_io, input, n_clusters,n_hidden,n_c_hidden,learn_type,nonlin):
    ...
    for c, context in enumerate(contexts):
        with tf.variable_scope(context):
            context_layers[c] = single_mlp_network(input, [n_io[0]] + n_c_hidden + [n_io[1]], nonlin[1]) #switch # nonlin_final = 'tanh'
    if nonlin[1] == 'linear':
...
def single_mlsp_network(input, n_hidden, nonlin = 'tanh', nonlin_final = 'linear', mult = 0.5):
...
def single_mlpsc_network(input, n_hidden, nonlin = 'tanh', nonlin_final = 'linear', simp = 0, mult = 0.5):
    ...
    for i, size in enumerate(zip(size_progression[:-1], size_progression[1:])):
        ...
        if i > 0:
            weights_sc = tf.get_variable("weights_s%s_layer%s"%(simp+1,str(i)),(size[1],n_hidden[0]),initializer=mlp_initializer)
            biases_sc = tf.get_variable("biases_s%s_layer%s"%(simp+1,str(i)), (size[1], 1),initializer=mlp_initializer)
            hidden_layer += apply_nonlinearity(input, weights_sc, biases_sc, nonlinearity)
    return hidden_layer

def single_mlpec_network(input, n_hidden, nonlin = 'tanh', nonlin_final = 'linear', simp = 0, mult = 0.5):
    mlp_initializer = tf.random_uniform_initializer(mult*-1.0,1.0*mult)
    hidden_layer = input
    size_progression = n_hidden
    output = 0
    for i, size in enumerate(zip(size_progression[:-1], size_progression[1:])):
        ...
        if i == len(size_progression)-2: 
            nonlinearity = nonlin_final
        if i < len(n_hidden)-2:
            weights_sc = tf.get_variable("weights_s%s_layer%s"%(simp+1,str(i)),(n_hidden[-1], size[0]),initializer=mlp_initializer)
            biases_sc = tf.get_variable("biases_s%s_layer%s"%(simp+1,str(i)), (n_hidden[-1], 1),initializer=mlp_initializer)
            output += apply_nonlinearity(hidden_layer, weights_sc, biases_sc, nonlinearity)
        weights = tf.get_variable("weights_s%s_layer%s"%(simp,str(i)),(size[1], size[0]),initializer=mlp_initializer)
        biases = tf.get_variable("biases_s%s_layer%s"%(simp,str(i)), (size[1], 1),initializer=mlp_initializer)
        hidden_layer = apply_nonlinearity(hidden_layer, weights, biases, nonlinearity)
    output += hidden_layer 
...
def single_mlp_network(input, n_hidden, nonlin = 'tanh', nonlin_final = 'linear', simp = 0, mult = 1.0, resnet = 'off'):
    mlp_initializer = tf.random_uniform_initializer(mult*-1.0,1.0*mult)
    hidden_layer = input
    size_progression = n_hidden
    for i, size in enumerate(zip(size_progression[:-1], size_progression[1:])):
        ...
        if i == len(size_progression)-2: 
            nonlinearity = nonlin_final
        if resnet == 'off' or i == len(size_progression)-2:
            hidden_layer = apply_nonlinearity(hidden_layer, weights, biases, nonlinearity)
        else:
            if size[1] == size[0]:
                hidden_layer = apply_nonlinearity(hidden_layer, weights, biases, nonlinearity) + hidden_layer
            if size[1] < size[0]:
                hidden_layer = extra_zeros(apply_nonlinearity(hidden_layer, weights, biases, nonlinearity),size) + hidden_layer
            if size[1] > size[0]:
                hidden_layer = apply_nonlinearity(hidden_layer, weights, biases, nonlinearity) + extra_zeros(hidden_layer,size)
    return hidden_layer

def extra_zeros(vector,size):
    number_zeros = np.abs(size[1]-size[0])
    zero_vector = tf.zeros((number_zeros, tf.shape(vector)[1]))
    vector_with_zeros = tf.concat([vector, zero_vector], 0)
...
def apply_nonlinearity(hidden_layer, weights, biases, nonlinearity):
    ...
    if nonlinearity == 'sin':
        hidden_layer = tf.math.sin(tf.matmul(weights,hidden_layer) + biases)   
    return hidden_layer
...
def choose_optimization(loss_runs, train_var_lists, learn_vars, lr_vars):
    ...
    if lr_decay_type == 'no_decay':
        ...
        learning_rate = [tf.minimum(loss_run*lr_start,0.01) for loss_run in loss_runs]
    elif lr_decay_type == 'by_loss2':
        learning_rate = [tf.minimum(loss_run*loss_run*lr_start,0.05) for loss_run in loss_runs]
    elif lr_decay_type == 'by_loss_sq':
        learning_rate = [tf.minimum(tf.sqrt(loss_run)*lr_start,0.05) for loss_run in loss_runs]
    # Use Adam or RMSProp
    if optim == 'RMSProp':
    ...
    if optim == 'SGD':
        train_step_runs = [tf.train.GradientDescentOptimizer(lr).minimize(loss_run, var_list = train_var_list, global_step=global_step) for lr, loss_run, train_var_list in zip(learning_rate,loss_runs,train_var_lists)]
    if optim == 'Adagrad':
        train_step_runs = [tf.train.AdagradOptimizer(lr).minimize(loss_run, var_list = train_var_list, global_step=global_step) for lr, loss_run, train_var_list in zip(learning_rate,loss_runs,train_var_lists)]
...
def tf_get_norm(val, type):
    if type == 'L1':
        norm = tf.abs(val)
    if type == 'L1_sqrt':
        norm = tf.sqrt(tf.abs(val))
    if type == 'L1_4rt':
        norm = tf.sqrt(tf.sqrt(tf.abs(val)))
    if type == 'L2':
        norm = tf.square(val)
    if type == 'L4':
        norm = tf.square(tf.square(val))
    return norm
...
def define_loss(y_, y, c, f, learn_type, train_state, n_clusters, norm_type):
    ...
    reg_loss = define_regularization_loss(learn_type, 0.0)
    #loss_deriv = define_loss_deriv(y_,y, np.array((train_state[4].shape)), train_loss_deriv = True)
    loss_min = tf.reduce_sum(tf.reduce_min(tf_get_norm(y_-f, norm_type),0)) + reg_loss
    loss_guess = tf.reduce_sum(tf_get_norm(y_-y, norm_type)) #+ reg_loss #+ loss_deriv
    loss_runs = [loss_guess+reg_loss] #[loss_guess]
    if learn_type == 'initialize': 
        loss_runs = [loss_min + reg_loss]
    ...
    return loss_runs, reg_loss

def define_regularization_loss(learn_type, mult = 0.001):
    train_var_reg = tf_flat_vars([v for v in tf.trainable_variables() if "biases" not in v.name])
    #train_var_reg = tf_flat_vars(tf.trainable_variables())
    #minval, mult1, mult2 = 0.3, 0.1, 0.00001
    #reg_loss = tf.reduce_mean(tf.minimum(tf.sqrt(tf.abs(train_var_reg)),minval))*mult1+tf.reduce_mean(tf.minimum(minval,tf.sqrt(tf.abs(train_var_reg))))*mult2
    reg_loss = tf.reduce_mean(tf_get_norm(train_var_reg,'L2')*mult) #+ tf_get_norm(train_other_var_reg,'L1')*mult*0.01
    if learn_type == 'context': 
        train_var_reg = tf_flat_vars([v for v in tf.trainable_variables() if "context" in v.name and "biases" not in v.name])
        reg_loss = tf.reduce_mean(tf_get_norm(train_var_reg,'L2')*mult)  #only for regularization   
    if learn_type == 'segment' or learn_type == 'init_f' or learn_type == 'init_c':
        ...
        reg_loss =  tf.reduce_mean(tf_get_norm(train_var_reg,'L1')*mult*0.001)
    if learn_type == 'sft' or learn_type == 'sft2' or learn_type == 'sft3' or learn_type == 'mlsp':
        train_var_reg, train_other_var_reg = tf_flat_vars([v for v in tf.trainable_variables() if "s0" in v.name and "biases" not in v.name]) #only for regularization
        reg_loss = tf.reduce_mean(tf_get_norm(train_var_reg,'L1_sqrt')*mult) #+ tf_get_norm(train_other_var_reg,'L1')*mult*0.01
        train_var_reg, train_other_var_reg = tf_flat_vars([v for v in tf.trainable_variables() if "s1" in v.name and "biases" not in v.name]) #only for regularization
        reg_loss = tf.reduce_mean(tf_get_norm(train_var_reg,'L1_sqrt')*mult*10) #+ tf_get_norm(train_other_var_reg,'L1')*mult*0.01
        train_var_reg, train_other_var_reg = tf_flat_vars([v for v in tf.trainable_variables() if "s2" in v.name and "biases" not in v.name]) #only for regularization
        reg_loss = tf.reduce_mean(tf_get_norm(train_var_reg,'L1_sqrt')*mult*100) #+ tf_get_norm(train_other_var_reg,'L1')*mult*0.01
        train_var_reg, train_other_var_reg = tf_flat_vars([v for v in tf.trainable_variables() if "s3" in v.name and "biases" not in v.name]) #only for regularization
        reg_loss = tf.reduce_mean(tf_get_norm(train_var_reg,'L1_sqrt')*mult*1000) #+ tf_get_norm(train_other_var_reg,'L1')*mult*0.01
    return reg_loss
...
def get_mini_batch(batch_ratio, batch_all_inputs, batch_all_ys, network_vars, norm_type, type):
    n_all = batch_all_ys.shape[1]
    batch_size = int(batch_ratio*n_all)
    randinds = get_random_inds(n_all, batch_size)    
    if type == 'random':     
        inds = randinds #get_random_slice(n_all,batch_size)
        #print(inds)
    if type == 'high_error':
        inds = get_high_error_inds(randinds.shape[0], int(batch_ratio*randinds.shape[0]), batch_all_inputs[:,randinds], batch_all_ys[:,randinds], network_vars, norm_type, 'high')
    if type == 'above_error':
        inds = get_high_error_inds(randinds.shape[0], int(batch_ratio*randinds.shape[0]), batch_all_inputs[:,randinds], batch_all_ys[:,randinds], network_vars, norm_type, 'above')
    batch_inputs, batch_ys = batch_all_inputs[:,inds], batch_all_ys[:,inds]
    return batch_inputs, batch_ys

def get_high_error_inds(n_all, batch_size, batch_all_inputs, batch_all_ys, network_vars, norm_type, type):
    sess, input, y_, f, c, y, loss_runs, reg_loss, train_step_runs, learning_rate = network_vars
    train_dict = {input: batch_all_inputs, y_:batch_all_ys}
    predictions = sess.run(y, feed_dict=train_dict)
    #if norm_type == 'L2':
        #error = np.sum(np.square(batch_all_ys - predictions),0)
        #print(error[:4],error[-4:])
    #if norm_type == 'L1':
    error = np.sum(np.abs(batch_all_ys - predictions),0)
    if type == 'above':
        possible_inds = np.arange(n_all)
        inds = possible_inds[error>0.0001]
    if type == 'high':
        inds = np.random.choice(n_all, batch_size, p=error/np.sum(error))
    return inds

def get_random_slice(n_all,randlen):
    test = np.arange(n_all)
    randint = np.random.randint(0,test.shape[0])
    #for randint in np.arange(test.shape[0]):
    if test.shape[0] > randint+randlen-1:
        inds = test[randint:randint+randlen]
    else:
        inds = np.hstack((test[randint:],test[:randlen-(test.shape[0]-randint)]))
    return inds

def get_random_inds(n_all, batch_size):
    inds = np.random.randint(0,n_all-1,batch_size)
    return inds

def train_network(network_vars, learn_vars, train_state, test_state, batch_ratio, num_epochs, norm_type):
    ...
    total_weights = np.zeros((num_epochs,num_variables))
    losses = [0]
    print('Training network...')
    #sess.run([v for v in tf.global_variables() if "global_step" in v.name][0].assign(0))
    j_printout = 100
    slope = 100
    batch_all_inputs_unshuffled, batch_all_ys_unshuffled = get_batch(1.0, train_state)
    batch_all_test_inputs_unshuffled, batch_all_test_ys_unshuffled = get_batch(1.0, test_state)
    inds = np.arange(batch_all_ys_unshuffled.shape[1])
    inds_test = np.arange(batch_all_test_ys_unshuffled.shape[1])
    np.random.shuffle(inds)
    np.random.shuffle(inds_test)
    batch_all_inputs, batch_all_ys = batch_all_inputs_unshuffled[:,inds], batch_all_ys_unshuffled[:,inds]
    batch_all_test_inputs, batch_all_test_ys = batch_all_test_inputs_unshuffled[:,inds_test], batch_all_ys_unshuffled[:,inds_test]
    train_dict = {input: batch_all_inputs, y_:batch_all_ys}
    test_dict = {input: batch_all_test_inputs, y_:batch_all_test_ys}
    print('batch all size = ',str(batch_all_ys.shape[1]))
    starttime = time()
    for j in range(0,num_epochs):
        #if j == 0 or (j % 20 == 0 and batch_ratio !=1.0):
        if j == 0:
            batch_test_inputs, batch_test_ys = get_mini_batch(batch_ratio, batch_all_test_inputs, batch_all_test_ys, network_vars, norm_type, 'random')
            subset_test = {input: batch_test_inputs, y_:batch_test_ys}
        if batch_ratio != 1.0 or j == 0:
            batch_inputs, batch_ys = get_mini_batch(batch_ratio, batch_all_inputs, batch_all_ys, network_vars, norm_type, 'random')
            subset = {input: batch_inputs, y_:batch_ys}
            bs = batch_inputs.shape[1]
        #if j == 0 or (j % 10 == 0 and batch_ratio !=1.0):
        #    batch_inputs, batch_ys = get_batch(batch_ratio, train_state)
        #    subset = {input: batch_inputs, y_:batch_ys}
        if j % j_printout == 0:
            losses = [sess.run(loss_run, feed_dict=train_dict) for loss_run in loss_runs]
            test_loss = sess.run(loss_runs[0], feed_dict=test_dict)
            reg_loss_val = sess.run(reg_loss, feed_dict=test_dict)
            lr_run = sess.run(learning_rate, feed_dict=subset)
        total_error[j] = sess.run(loss_runs[0], feed_dict=subset_test)
        total_weights[j] = np.hstack([np.ndarray.flatten(item) for item in sess.run([var for var in tf.trainable_variables()])])
        ...
        if j % j_printout == 0:
            ...
            m, s = divmod(t_s, 60)
            print('%s/%s'%(j,num_epochs), "Test loss vec is", losses, test_loss-reg_loss_val, "and lr", lr_run,'bs',bs,'eta %sm %ss'% (m,s),'cs',slope,'/',desired_slope) 
            #print('%s/%s'%(j,num_epochs), "Loss vec is", losses, test_losses, "and lr", lr_run,'bs',bs,'eta %sm %ss'% (m,s),'cs',slope,'/',desired_slope) 
    network_vars = sess, input, y_, f, c, y, loss_runs, reg_loss, train_step_runs, learning_rate
    return network_vars, total_error, total_weights, num_epochs_print
...
def check_convergence(savename, network_vars, n_clusters, train_state, learn_type, n_hidden):
    sess, input, y_, f, c, y, loss_runs, reg_loss, train_step_runs, learning_rate = network_vars
    ...
    if learn_type == 'initialize' or learn_type == 'context':
        ...
        n_clusters_network, save_vars = (n_clusters, tf.trainable_variables()) 
    ...
    if n_hidden[0] != [] and n_hidden[0] != [1]:
