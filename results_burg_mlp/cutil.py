...
def perceive(label = 'Burgers', stride_t = 1, stride_x = 1, slope = 0.0, angle = 0.0):
    if label == 'Identity':
       state, scales = construct_linear_data(stride_t, stride_x, 'identity')
    if label == 'Reverse':
       state, scales = construct_linear_data(stride_t, stride_x, 'reverse')
    if label == 'Affine':
       state, scales = construct_linear_data(stride_t, stride_x, 'affine')
    if label == 'Fake Steps':
       state, scales = construct_fake_steps(stride_t, stride_x, slope, angle)
    if label == 'Burgers':
...
def import_burgers_data(stride_t = 1, stride_x = 1):
    # Define mu
    mu = np.array((1.0,3.0,4.0,2.0,5.0))
    n_mu = mu.shape[0]
    # Code here for importing data from file
    output =  read_me(['burg/snaps_0p05_0p02_%s.dat'%str(int(m)) for m in mu]).T
    # Define x and t
...
def construct_linear_data(stride_t = 1, stride_x = 1, type = 'affine'):
    if type == 'identity' or type == 'reverse':
        n_mu, n_t, n_x = (6, 6, 100)
    else:
        n_mu, n_t, n_x = (6, 20, 100)
    ts = np.linspace(-1.0, 1.0, n_t)
    xs = np.linspace(-1.0, 1.0, n_x)
    mus = np.array((-1,0,0.75,-0.5,0.5,1.0))
    stimuli = mus.reshape(n_mu,1)
    temporal_points = ts.reshape(n_t,1)
    spatial_points = xs.reshape(n_x,1)
    clips = np.ones((n_mu, n_t, n_x, 1))
    for m, mu in enumerate(stimuli):
        for j, t in enumerate(temporal_points):
            if type == "reverse":
                for i, x in enumerate(np.flip(spatial_points)):
                    clips[m,j,i]= x
            else:
                for i, x in enumerate(spatial_points):
                    if type == "identity":
                        clips[m,j,i]= x
                    if type == "affine":
                        clips[m,j,i]= 0.5*x+0.3*t+0.2*mu+0.0 # bias has no effect except harder to check weights
    raw_state = stimuli, temporal_points, spatial_points, clips
    state, scales = scale_stride(raw_state, stride_t, stride_x)
    return state, scales

def construct_fake_steps(stride_t = 1, stride_x = 1, slope = 0.0, angle = 0.0):
    # Construct fake input data
    n_mu, n_t, n_x = (5, 5, 250)
    t = np.linspace(1.0, 5.0, n_t)
    x = np.linspace(-50.0, 150.0, n_x)
    mu = np.array([0.0,0.5,1.0,0.25,0.75])
    # Construct fake function data
    ind_center, ind_step0, ind_step1 = (int(n_x/2), int(n_x/20), int(n_x/10))
    ind_step05 = int((ind_step0+ind_step1)/2)
    ind_step025 = int((ind_step0+ind_step05)/2) 
    ind_step075 = int((ind_step05+ind_step1)/2) 
    yinit0, yinit1 = (10.0, 6.0)
    yinit05 = (yinit0+yinit1)/2
    yinit025 = (yinit0+yinit05)/2
    yinit075 = (yinit05+yinit1)/2
    yend0, yend1 = (2.0, 4.0)
    yend05 = (yend0+yend1)/2
    yend025 = (yend0+yend05)/2
    yend075 = (yend05+yend1)/2
    output = np.ones((n_mu*n_t,n_x))
    i = 0
    for indstep, yinit, yend in zip([ind_step0,ind_step05,ind_step1,ind_step025,ind_step075],[yinit0,yinit05,yinit1,yinit025,yinit075],[yend0,yend05,yend1,yend025,yend075]):
        for j in [2, 1, 0, -1, -2]:
    ...
    output2 = output2.reshape(n_mu, n_t, n_x, 1)[:,:,20:-20,:]
    stimuli = stimuli.reshape(n_mu,1)
    temporal_points = temporal_points.reshape(n_t,1)
    spatial_points = spatial_points.reshape(n_x,1)[20:-20,:]
...
def plot_it(v, v_indices = np.array([17]), i=0, flag = 'show', title = None, pltaxis = None, savename = 'plt'):
    fig = plt.figure(figsize=(6,6)) #12,6
    plt.title(title,size=14)
    #jet = plt.get_cmap('jet')
    if (v_indices == np.array([17]))[0]:
         vspace = np.arange(v.shape[0])
         plt.plot(vspace, v,'k', lw=2)
    else:
         vspace = v_indices
         plt.plot(vspace, v,'k', lw=2)
    if pltaxis != None:
         plt.axis(pltaxis)
    plt_save_or_show(plt, flag, savename)

def plot_two(v1, v1_indices, v2, v2_indices, i=0, flag = None, title = None, pltaxis = None, savename = 'plt'):
    #color = 'bgrcmyk'
    fig = plt.figure(figsize=(6,6)) #12,6
    plt.title(title,size=14)
    plt.plot(v2_indices, v2, 'k', lw=2)
    plt.plot(v1_indices, v1, 'b--', lw=2)
    if pltaxis != None:
         plt.axis(pltaxis)
    plt_save_or_show(plt, flag, savename)
...
def plot_only_function_network_results(network_vars, state, n_clusters, stimuli_range, train_dict, time_plot, pflag):
    ...
    print(f_run_plot.shape, reconstructions.shape, clips.shape, spatial_points.shape)
...
def plot_segment_network_results(network_vars, state, n_clusters, stimuli_range, train_dict, time_plot, pflag):
    ...
    print(f_run.shape, clips.shape)
    [plot_it(np.vstack((f_run[:,j,time_plot,:], clips[j,time_plot,:,0])).T,spatial_points[:,0],0,'training_data',pflag) for j in range(stimuli_range)]
...
def plot_results(savename, network_vars, state, time_plot, pflag):
    stimuli, temporal_points, spatial_points, clips = state
    sess, input, y_, f, c, y, loss_runs, reg_loss, train_step_runs, learning_rate = network_vars
    ...
    plot_two(predictions[:,time_plot].T,spatial_points[:,0],clips[:,time_plot,:,0].T,spatial_points[:,0],0,pflag, savename = 'plt_guess_overlay_'+savename)
    plot_it(np.vstack((predictions[:,time_plot])).T,spatial_points[:,0],0,pflag, savename = 'plt_guess_'+savename)
    [plot_two(predictions[list(stimuli).index(stimulus),::nskip,:].T,spatial_points[:,0],clips[list(stimuli).index(stimulus),::nskip,:,0].T,spatial_points[:,0],0,pflag,savename='plt_overlay_%s_%s' % (stimulus[0],savename)) for stimulus in stimuli]
    [plot_it(clips[list(stimuli).index(stimulus),::nskip,:,0].T,spatial_points[:,0],0,pflag,savename='plt_truth_%s_%s' % (stimulus[0],savename)) for stimulus in stimuli]

def plot_all(savename, label, network_vars, state, n_clusters, stimuli_range, train_dict, time_plot, pflag, learn_type, num_epochs, num_epochs_print, total_error):
    if label == 'Bubble':
        ...
        plot_results(savename, network_vars, state, time_plot, pflag)
...
def plot_cfd_results(savename, network_vars, state, time_plot, pflag):
    ...
    #plot_cfd_4(clips[0,-1],pflag,'plt_0_truth_clips') # in ns, nt, nxy, nv
    if pflag == 'all_save':
        for i in np.arange(stimuli.shape[0]):
            plot_cfd_4(truth[i,-1],'save','plt_'+str(i)+'_truth') #[-3,-2] for test set
            plot_cfd_4(predictions[i,-1],'save','plt_'+str(i)+'_guess_'+savename)
            plot_cfd_4(np.square(predictions[i,-1]-truth[i,-1]),'save','plt_'+str(i)+'_error_'+savename)
    else:
        plot_cfd_4(truth[0,-1],pflag,'plt_0_truth') #[-3,-2] for test set
        plot_cfd_4(predictions[0,-1],pflag,'plt_0_guess_'+savename)

def print_l2_loss(network_vars, state, train_dict):
    sess, input, y_, f, c, y, loss_runs, reg_loss, train_step_runs, learning_rate = network_vars
    train_loss = sess.run(tf.sqrt(tf.reduce_mean(tf.square(y_-y))),train_dict)
    chosen_stimuli, stimuli_range = (3,2)
    test_state = get_state_subset(chosen_stimuli, stimuli_range, state)
    batch_inputs, batch_ys = get_batch(1.0, test_state)
    forwardtime = time()
    test_loss = sess.run(tf.sqrt(tf.reduce_mean(tf.square(y_-y))),{input: batch_inputs, y_:batch_ys})
    print('Test forward pass took %s seconds'%(time()-forwardtime))
...
def print_QoI_values(network_vars, state, scale):
    ...
    #print('QoIst =',QoIst)
    #print('QoIsg =',QoIsg)
    Pct_Error = np.mean(np.abs(unscale(truth[3:,:,:],scale)-unscale(guesses[3:,:,:],scale))/7.546209)*100 #% Error
    #Pct_Error = np.mean(np.abs(unscale(truth[3:,:,:],scale)-unscale(guesses[3:,:,:],scale))/0.00511911)*100 #% Error
    #0.00511911 0.000248076
...
def calc_QoI_Pct_Error(QoIst, QoIsg):
    return np.mean(np.divide(np.abs(QoIst[:,-2:]-QoIsg),7.546209))*100
