...
def plot_results(savename, network_vars, state, time_plot, pflag):
    ...
    plot_it(np.vstack((predictions[:,time_plot], clips[:,time_plot,:,0])).T,spatial_points[:,0],0,pflag, savename = 'plt_guess_overlay_'+savename)
    plot_it(np.vstack((predictions[:,time_plot])).T,spatial_points[:,0],0,pflag, savename = 'plt_guess_'+savename)
    [plot_it(np.vstack((predictions[list(stimuli).index(stimulus),::nskip,:],  clips[list(stimuli).index(stimulus),::nskip,:,0])).T,spatial_points[:,0],0,pflag,savename='plt_overlay_%s_%s' % (stimulus[0],savename)) for stimulus in stimuli]
